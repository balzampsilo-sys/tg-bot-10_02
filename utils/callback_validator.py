...
class CallbackValidator:
    ...
    # Версия текущих callback (из config)
    CURRENT_VERSION = CALLBACK_VERSION
    
    # Время жизни callback сообщений (из config)
    ...
    async def validate_callback(
        ...
        # Убедимся что даты с timezone
        if message_date.tzinfo is None:
            message_date = TIMEZONE.localize(message_date)
        ...
        if message_age.total_seconds() / 3600 > cls.MESSAGE_TTL_HOURS:
            ...
            return ValidationResult(
                is_valid=False,
                error_message=f"⚠️ Сообщение устарело ({hours_old}ч)\n\nНачните заново с /start",
                should_disable_buttons=True,
                should_clear_state=True
            )
...
def validate_callback(expected_parts: int = None, check_version: bool = True):
    """
    Декоратор для автоматической валидации callback handlers
    
    Usage:
        @router.callback_query(F.data.startswith("time:"))
        @validate_callback(expected_parts=3)
    ...
    def decorator(handler):
        async def wrapper(callback: CallbackQuery, state: FSMContext, *args, **kwargs):
            result = await CallbackValidator.validate_callback(
                callback, state, expected_parts, check_version
            )
            
            if not result.is_valid:
                await CallbackValidator.handle_validation_error(callback, state, result)
                return
